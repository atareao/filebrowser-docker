...
install_filemanager()
{
	...
	*)
	...
	if type -p curl >/dev/null 2>&1; then
		curl -fsSL "$filemanager_url" -o "$filemanager_file"
	elif type -p wget >/dev/null 2>&1; then
		wget --quiet "$filemanager_url" -O "$filemanager_file"
	else
	...
	case "$filemanager_file" in
		*.zip)    unzip -o "$filemanager_file" "$filemanager_bin" ;;
		*.tar.gz) tar -xzf "$filemanager_file" "$filemanager_bin" ;;
	esac
	rm "$filemanager_file"
	chmod +x "$filemanager_bin"
	docker build -t atareao/filebrowser:$filemanager_tag .
	docker image push atareao/filebrowser:$filemanager_tag
}
